...
<fieldset><legend><b>Order Details</b></legend>
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method='get'>
			...
			<label for='payment'>Payment: </label><input type=radio name='payment' value='online' required> Online : Enter beneficiary UPI ID
			<input type=text name='phone' class='formInputItem'><br><input type=radio name='payment' value='COD' required> Cash On Delivery<br>
	...
	<?php
		if(isset($_GET['submit'])){
			...
			$result=mysqli_query($conn,$sql);
			echo "<script>alert('Order placed.\\nThe products will be delivered within 1 hour.');</script>";
